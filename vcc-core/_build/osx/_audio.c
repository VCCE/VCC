...
struct audbuffer_t
{
	...
	uint32_t		iDataIndex;							// current position/size of data in this buffer
		...
		if ( slinklistGetCount(&pOSXAudio->lstBuffersReadyToPlay) > 0 )
		{
			   ...
			   )
			{
				...
				if ( pBuffer != NULL )
				{
					...
					// fill OpenAL buffer 
					alBufferData(pBuffer->alBuffer, AL_FORMAT_STEREO16, (char *)pBuffer->cData, (ALsizei)pBuffer->iDataIndex<<2, iRateList[pAudio->CurrentRate]);
					assert((error = alGetError()) == AL_NO_ERROR);
