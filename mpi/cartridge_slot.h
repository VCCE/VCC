...
namespace vcc { namespace modules { namespace mpi
{
	...
	public:
		...
		using cartridge_ptr_type = ::std::shared_ptr<::vcc::core::cartridge>;
	...
	public:
		...
		const catalog_id_type& catalog_id() const
		{
		...
		{
			cartridge_line_state_ = state;
		}

		bool line_state() const
		{
			return cartridge_line_state_;
		}
		...
		void enumerate_menu_items(AppendCartridgeMenuModuleCallback append_menu_item_callback) const
		{
			for (const auto& item : menu_items_)
			{
				append_menu_item_callback(item.name.c_str(), item.menu_id, item.type);
			}
		}
	...
	private:
		...
		bool cartridge_line_state_ = false;
	};
